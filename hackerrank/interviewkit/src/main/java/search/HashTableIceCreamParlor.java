...
public class HashTableIceCreamParlor {
    ...
    static void whatFlavors(int[] cost, int money) {
        ...
        for (int i=0; i< cost.length; i++) {
            if (costMap.containsKey(cost[i])) {
                //found the solution. return from here.
                System.out.println((costMap.get(cost[i])+1) +" "+(i+1));
                return;
            }
            else {
                int remainingCost = money - cost[i];
                costMap.put(remainingCost, i);
            }
    ...
    public static void hashtableIceCreamParlor(String[] args) {
